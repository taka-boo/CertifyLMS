...
// ============================================================
// 受講生専用 — 質問掲示板(スレッドの投稿・編集・削除・解決状態切替…コーチは実行不可)
// ============================================================
Route::middleware(['auth', 'role:student', 'active-learning'])
    ->prefix('qa-board')
    ->name('qa-board.')
...
// ============================================================
// 管理者専用 — 質問掲示板(全資格横断、削除のみ)
// ============================================================
Route::middleware(['auth', 'role:admin'])->prefix('admin')->group(function () {
    Route::get('qa-board', [QaThreadController::class, 'adminIndex'])->name('admin.qa-board.index');
    Route::get('qa-board/{thread}', [QaThreadController::class, 'adminShow'])->name('admin.qa-board.show');
    Route::delete('qa-board/{thread}', [QaThreadController::class, 'adminDestroy'])->name('admin.qa-board.destroy');
    Route::delete('qa-board/{thread}/replies/{reply}', [QaReplyController::class, 'adminDestroy'])->name('admin.qa-board.replies.destroy');
});
